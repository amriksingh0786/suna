...
async def get_current_user_id_from_jwt(request: Request) -> str:
    ...
    try:
        ...
        logger.error(f"An unexpected error occurred during token processing: {str(e)}")
        raise HTTPException(
            status_code=500,
...
async def get_current_user_claims_from_jwt(request: Request) -> Optional[dict]:
    """
    Extract and verify the JWT from the Authorization header, then return its payload (claims).
    
    Args:
        request: The FastAPI request object
        
    Returns:
        Optional[dict]: The decoded JWT payload (claims dictionary), or None if auth is disabled.
        
    Raises:
        HTTPException: If no valid token is found or if the token is invalid when auth is enabled.
    """
    logger.debug("Attempting to get user claims from JWT...")
    if config.DISABLE_AUTH:
        logger.debug(f"Auth is disabled, returning None for user_claims.")
        # For disabled auth, specific endpoints might inject a dev user_id or mock claims directly.
        # This function will return None, and the caller can decide how to handle it.
        return None 
    ...
    logger.debug(f"Attempting to decode token: {token}")
    try:
        # For now, we are not verifying the signature, issuer, or audience.
        # This is because the token is generated by the X-API and assumed to be valid.
        # In a more robust setup, you would verify these using the X-API's public key / secret.
        payload = jwt.decode(token, options={"verify_signature": False})
        logger.debug(f"Token decoded successfully. Payload: {payload}")
        
        if not payload.get('id'): # Ensure at least the ID is present, as it's used as user_id
            logger.warning("User ID ('id' claim) not found in token payload.")
            raise HTTPException(
                status_code=401,
                detail="Invalid token payload: ID missing",
            )
        return payload
    except PyJWTError as e:
        logger.error(f"Invalid token: {str(e)}")
